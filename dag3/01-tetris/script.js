...
document.addEventListener("keydown", (e) => {
    ...
    if(e.key === "ArrowRight"){// && parseInt(box.style.left) + boxWidth < width){
        box.style.left = parseInt(box.style.left) + step + "px";
    }
    if(e.key === "ArrowLeft"){ //&& parseInt(box.style.left) > 0){
        box.style.left = parseInt(box.style.left) - step + "px";
    }
    if(e.key === "ArrowUp"){ //&& parseInt(box.style.top) > 0){
        box.style.top = parseInt(box.style.top) - step + "px";
    }
    if(e.key === "ArrowDown") //&& parseInt(box.style.top) + boxHeight < height)
    {
        box.style.top = parseInt(box.style.top) + step + "px";
    }
    if(e.key === " "){
    ...
    if(e.key === "c"){
        box.style.background = ChooseColor();
    }
});
